...
if __name__ == '__main__':
    ...
    if argc == 2:
        ...
        log.debug('http pacp/pcapng file: %s' % pcap_file)
        ...
        for filename in tmp_list:
            if (filename.endswith('.pcap') or filename.endswith('.pcapng')) and os.path.isfile(filename):
                pcap_file = filename
                log.debug('http pacp file: %s' % pcap_file)
                http_filter(pcap_file)

    if pcap_file == '' or not os.path.isfile(pcap_file):
        log.error('http pacp/pcapng file: %s noooon exist, exit...' % pcap_file)
        sys.exit(1)



    #log.info('request_info:{0}'.format(g_dict_request)) 
